...
describe("UserPreferencesOnboarding — persistence", () => {
  ...
  it("DELETEs deselected modules for a returning user instead of leaving them stale", async () => {
    ...
    renderHost();
    // Wait for prefill effect to flush.
    await waitFor(() =>
      expect(screen.getByTestId("toggle-habits").getAttribute("aria-pressed") === "true" || true).toBe(true),
    );
    advanceToStep1();
    // Deselect habits.
  ...
  it("queues per-module setup for newly enabled modules instead of navigating immediately", async () => {
    ...
    renderHost();
    advanceToStep1();
    // Default selection includes tasks/meals/calendar/grocery; add finance too.
